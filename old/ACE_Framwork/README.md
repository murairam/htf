...
The ACE Framework supports multiple LLM providers:

| Provider | Environment Variable | Default Model | Notes |
|----------|---------------------|---------------|-------|
| **OpenAI** | `OPENAI_API_KEY` | `gpt-4` | Full support for chat and vision |
| **Blackbox AI** | `BLACKBOX_API_KEY` | `blackboxai` | OpenAI-compatible API, cost-effective |
| **Google Gemini** | `GOOGLE_API_KEY` | `gemini-pro` | Google's latest models |
| **Anthropic Claude** | `ANTHROPIC_API_KEY` | `claude-3-opus` | Advanced reasoning capabilities |

To use a specific provider, set the `provider` parameter in your configuration:
