...
class LLMConfig:
    ...
    provider: str = "blackbox"  # Changed from "openai" to "blackbox"
    model: str = "blackboxai/deepseek/deepseek-chat"  # Best for reasoning and analysis
    vision_model: str = "blackboxai/openai/gpt-4o"  # For image analysis
    temperature: float = 0.0
    max_tokens: int = 2048  # Reduced from 4096 to avoid context length errors
    stream: bool = True
    ...
    def __post_init__(self):
        if self.api_key is None:
            if self.provider == "openai":
                ...
                # Use BLACKBOX API key
                self.api_key = BLACKBOX_API_KEY if BLACKBOX_API_KEY else None
                if not self.api_key:
